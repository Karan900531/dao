...
import { getUserBalanceing } from "../utils/tokenContract";
...
const { hourss, minutess, secondss } = formatTimes(minreward);
  ...
  const handleGetData = useCallback(async () => {
    try {
      if (!address || !chain) return;
      setBalance(await getUserBalanceing(address, chain));
...
Staking / <span style={{color:'#ffba00'}}>Multichain Staking Pool 1</span>
            </h2>
            </Link>

            <h2 style={{color:'#ffba00',paddingLeft:"10px"}}>
              Multichain Staking Pool 1</h2>
          </div>
        </div>
      </div>
            ...
            <div className="divthreemains">
              <div className="divthreemain">
                <div className="divthree">
                  ...
                  <p style={{fontSize:14,textAlign:'right'}}><i className="fa fa-wallet"></i> : {balance.toString()} 8Bit</p>
                  <div>
  ...
  <div className="web3-btn">
    <button onClick={handleUnstake} >
      ...
      <div className="stakeblock">
          ...
          <p>8Bit Holding : <span style={{fontWeight:900}}>{new Intl.NumberFormat("en-US", {
                minimumFractionDigits: 0,
                maximumFractionDigits: 4,
              }).format(balance)}{" "} 8Bit</span></p>
