...
import { stakingAddressOld2, tokenAddress, RPC_URL } from "../utils/address";

import StakingAbi from "../utils/abis/oldStaking2.json";
...
export default function Farm() {
  ...
  const [calculateref, calculaterefs] = useState(0);
  const [claimedRefs, claimedRefo] = useState(0);
...
const { hourss, minutess, secondss } = formatTimes(minreward);
  ...
  const handleGetData = useCallback(async () => {
    try {
      ...
      calculaterefs(await calculateRef(address, chain));
      claimedRefo(await claimedRef(address, chain));
  ...
  const handleStake = async () => {
    try {
      ...
      // Check if the user owns NFT
      const contract = new ethers.Contract(
        stakingAddressOld2[chain as keyof typeof stakingAddressOld2],
        StakingAbi,
        await getSigner(address)
      );
  
      const doesOwnNFT = await contract.ownsNFT(address);
      
      if (!doesOwnNFT) {
        setStatus({ status: "ERROR", message: "You don't own the required NFT to stake." });
        return;
      }
  ...
  const handleEarlyUnstake = async () => {
    try {
      if (!address) return alert("connect wallet");
      setStatus({ status: "PROCESSING", message: "Unstaking your amount..." });
      await EarlyunStakeToken(address, chain, amountU);
      setStatus({ status: "SUCCESS", message: "Unstaked successfully" });
    } catch (error) {
      console.log(error);
      setStatus({ status: "ERROR", message: "Transaction failed" });
...
Staking / <span style={{color:'#ffba00'}}>Pool 1</span>
            </h2>
            </Link>
...
<h6>Time Left for Unstake</h6>
  ...
  <div className="web3-btn" >
    <button onClick={handleEarlyUnstake} >
      Early Unstake
    </button>
  </div>
</div>
...
<h6 style={{marginTop:30}}>Your Claimed Reflection</h6>
                    <div className="timer" style={{marginBottom:0}}>
                    <h2>
            {new Intl.NumberFormat("en-US", {
                minimumFractionDigits: 0,
                maximumFractionDigits: 4,
