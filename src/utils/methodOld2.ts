import { stakingAddressOld2, tokenAddress, RPC_URL } from "./address";
import { ethers, formatEther, MaxUint256, parseEther } from "ethers";

import StakingAbi from "./abis/oldStaking2.json";
...
export const stakeToken = async (address: string, chainId: number, amount: string) => {
  ...
  const contract = new ethers.Contract(
    stakingAddressOld2[chainId as keyof typeof stakingAddressOld2],
    StakingAbi,
    signer
  );
  ...
  if (!doesOwnNFT) {
    console.log("User does not own NFT. Staking tokens not allowed.");
    return; // Exit the function if user does not own NFT
  }
